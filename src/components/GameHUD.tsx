  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-between pointer-events-none select-none">
      ...
      <div className="pt-8 w-full flex items-start justify-center relative">
        <p
          className="text-still-white/[0.72] text-sm tracking-[0.2em] uppercase font-light"
      ...
      {!centerHidden && (
        <div className="flex w-full max-w-[calc(100vw-2rem)] flex-col items-center gap-0 translate-y-[clamp(46px,12vh,100px)] landscape:translate-y-[clamp(28px,7vh,56px)]">
          <div className="relative flex w-full flex-col items-center gap-3 px-4">
            <div
              className="exhale-hud-backplate pointer-events-none absolute left-1/2 top-1/2 h-44 w-[min(24rem,88vw)] -translate-x-1/2 -translate-y-[52%] rounded-full bg-forest-night/[0.42] blur-2xl"
              aria-hidden="true"
            />
            <div className="relative flex h-16 w-full min-w-0 items-center justify-center">
              {previousPhase && (
                <h2
                  className="exhale-phase-label exhale-phase-out absolute inset-x-0 text-center text-[2.75rem] font-semibold leading-none tracking-[0.12em] text-still-white/[0.95] uppercase sm:text-5xl"
                  style={{
              ...
              <h2
                className={`exhale-phase-label absolute inset-x-0 text-center text-[2.75rem] font-semibold leading-none tracking-[0.12em] text-still-white/[0.95] uppercase sm:text-5xl ${previousPhase ? 'exhale-phase-in' : ''}`}
                style={{
                  ['--phase-opacity' as string]: labelOpacity,
                  textShadow: phaseLabelShadow,
                  opacity: labelOpacity,
            ...
            <div
              className="exhale-phase-timer mt-0 text-7xl font-thin leading-none tabular-nums text-still-white/[0.90] sm:text-8xl"
              style={{
      ...
      <div className="pb-[calc(7rem+env(safe-area-inset-bottom))] flex flex-col items-center gap-1.5">
        {!settled && (
          <p
            className="text-still-white/[0.70] text-xs tracking-[0.15em] uppercase font-light"
            style={{ textShadow: '0 1px 6px rgba(15,23,18,0.6)' }}
          >
            ~{minutesLeft} min remaining
          </p>
        )}
        <p className="hidden sm:block text-still-white/[0.60] text-xs tracking-[0.1em] font-light" aria-hidden="true">
